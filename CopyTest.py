    ...
    def choose_move(self, battle: pb.Battle):
        ...
        for move in self.current_poke.moves + list(range(len(battle.t1.poke_list))):
            ...
            if type(move) == int and self.validSwitch(move, True):
                score = self.minimax(new_battle, move, 1, False, float('-inf'), float('inf'))
...
ash = MinimaxTrainer("ash", team1, selection_function)

misty = pb.Trainer("misty", team1Copy, selection_function2)
